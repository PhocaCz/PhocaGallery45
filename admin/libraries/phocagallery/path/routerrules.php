...
class PhocaGalleryRouterrules extends \JComponentRouterRulesMenu
{
	...
	private function processForCategory(&$query)
	{


		// Do nothing when the query is not for a location
		if (empty($query['view']) || ($query['view'] != 'category' && $query['view'] != 'locationform')) {
			return;
		}
		...
		foreach ($this->lookup as $languageName => $items) {
			...
			if (!empty($items['location']) && array_key_exists($id, $items['category'])) {
			...
			if (!empty($items['locations']) && array_key_exists($id, $items['category'])) {
			...
			if (!empty($items['locations']) && array_key_exists(-1, $items['category'])) {
