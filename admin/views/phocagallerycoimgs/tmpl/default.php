...
echo '<th class="ph-date">'.Joomla\CMS\HTML\HTMLHelper::_('searchtools.sort',  		$OPT.'_DATE', 'a.date', $listDirn, $listOrder ).'</th>'."\n";
...
echo '<th class="ph-imagetitle">'.Joomla\CMS\HTML\HTMLHelper::_('searchtools.sort', $OPT.'_IMAGE', 'image_title', $listDirn, $listOrder ).'</th>'."\n";
...
if ($item->title == '') {
	$item->title = JText::_('COM_PHOCAGALLERY_NO_TITLE');
}

if ($canCreate || $canEdit) {
	$checkO .= '<a href="'. JRoute::_($linkEdit).'">'. $this->escape($item->title);
	if ($item->comment != '') {
